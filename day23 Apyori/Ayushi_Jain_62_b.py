# -*- coding: utf-8 -*-
"""
Created on Tue Jun 12 13:41:35 2018
...
for i in ass_cand_names:
    elec_lst = list(set(data["Party"][data["Name_of_Candidate"]==i]))
    if len(elec_lst) > 1:
...
for c,f in zip(ppl_lst,freq):
    votes = []
    for const in f:
        votes.append(data["Votes"][(data["Party"]==const) & (data["Name_of_Candidate"]==c)].mean())
    total.append(votes)
...
plt.title("Outcome of the next election after Changing Party")
plt.show()
